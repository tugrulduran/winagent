...
#include <hidapi.h>
#include <cstdint>
#include "AudezeSampler.h"

namespace audeze {
    AudezeSampler::AudezeSampler() {
        init();
    }

    void AudezeSampler::init() {
        hid_init();
    }

    std::string AudezeSampler::findDevicePath() {
    ...
    std::vector<uint8_t> AudezeSampler::sendCommandAndGetReport(hid_device *handle, const std::vector<uint8_t> &command) {
    ...
    uint8_t AudezeSampler::getBattery() {
        std::string path = findDevicePath();
        if (path.empty()) return 0xFF;

        hid_device *handle = hid_open_path(path.c_str());
        if (!handle) return 0xFE;
