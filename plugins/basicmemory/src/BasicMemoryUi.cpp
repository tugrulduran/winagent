...
static constexpr const char* kPluginId = "basicmemory";

QString BasicMemoryUi::stateToText(int32_t state) {
    switch (state) {
...
QString BasicMemoryUi::configPath() const {
...
BasicMemoryUi::BasicMemoryUi(WaHostApi* api, QWidget* parent)
    ...
    // Settings
    // =====================
    auto* grpSettings = new QGroupBox("Basic Memory Settings", this);
...
void BasicMemoryUi::loadFromDisk() {
    QFile f(configPath());
    if (!f.exists()) {
        spinInterval_->setValue(5000);
        return;
    }
    if (!f.open(QIODevice::ReadOnly)) {
        QMessageBox::warning(this, "Basic Memory", "Failed to open config file for reading.\n" + configPath());
        spinInterval_->setValue(5000);
        return;
    }
    ...
    if (perr.error != QJsonParseError::NoError || !doc.isObject()) {
        QMessageBox::warning(this, "Basic Memory", "Config JSON parse error.\n" + perr.errorString());
        spinInterval_->setValue(5000);
        return;
    }

    const QJsonObject root = doc.object();
    spinInterval_->setValue(root.value("intervalMs").toInt(5000));
}

bool BasicMemoryUi::saveToDisk() {
    ...
    if (!sf.open(QIODevice::WriteOnly)) {
        QMessageBox::warning(this, "Basic Memory", "Failed to open config file for writing.\n" + configPath());
        return false;
    }
    ...
    if (!sf.commit()) {
        QMessageBox::warning(this, "Basic Memory", "Failed to commit config file changes.\n" + configPath());
        return false;
    }
...
void BasicMemoryUi::refreshStatus() {
