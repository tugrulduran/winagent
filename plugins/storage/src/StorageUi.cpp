...
static constexpr const char* kPluginId = "storage";

QString StorageUi::stateToText(int32_t state) {
    switch (state) {
...
StorageUi::StorageUi(WaHostApi* api, QWidget* parent)
    ...
    // Settings
    // =====================
    auto* grpSettings = new QGroupBox("Storage Settings", this);
...
void StorageUi::loadFromDisk() {
    ...
    if (!f.open(QIODevice::ReadOnly)) {
        QMessageBox::warning(this, "Storage", "Failed to open config file for reading.\n" + configPath());
        spinInterval_->setValue(1000);
        return;
    }
    ...
    if (perr.error != QJsonParseError::NoError || !doc.isObject()) {
        QMessageBox::warning(this, "Storage", "Config JSON parse error.\n" + perr.errorString());
        spinInterval_->setValue(1000);
        return;
    }
...
bool StorageUi::saveToDisk() {
    ...
    if (!sf.open(QIODevice::WriteOnly)) {
        QMessageBox::warning(this, "Storage", "Failed to open config file for writing.\n" + configPath());
        return false;
    }
    ...
    if (!sf.commit()) {
        QMessageBox::warning(this, "Storage", "Failed to commit config file changes.\n" + configPath());
        return false;
    }
    return true;
}

void StorageUi::refreshStatus() {
