...
static constexpr const char* kPluginId = "media";

QString MediaUi::stateToText(int32_t state) {
    switch (state) {
...
MediaUi::MediaUi(WaHostApi* api, QWidget* parent)
    ...
    // Settings
    // =====================
    auto* grpSettings = new QGroupBox("Media Settings", this);
...
void MediaUi::loadFromDisk() {
    ...
    if (!f.open(QIODevice::ReadOnly)) {
        QMessageBox::warning(this, "Media", "Failed to open config file for reading.\n" + configPath());
        spinInterval_->setValue(1000);
        return;
    }
    ...
    if (perr.error != QJsonParseError::NoError || !doc.isObject()) {
        QMessageBox::warning(this, "Media", "Config JSON parse error.\n" + perr.errorString());
        spinInterval_->setValue(1000);
        return;
    }
...
bool MediaUi::saveToDisk() {
    ...
    if (!sf.open(QIODevice::WriteOnly)) {
        QMessageBox::warning(this, "Media", "Failed to open config file for writing.\n" + configPath());
        return false;
    }
    ...
    if (!sf.commit()) {
        QMessageBox::warning(this, "Media", "Failed to commit config file changes.\n" + configPath());
        return false;
    }
