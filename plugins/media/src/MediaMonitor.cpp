...
namespace media {
    ...
    MediaData MediaMonitor::getMedia() {
        ScopedWinRT winrt;
        if (!winrt.ok()) {
            return {L"", MEDIA_SOURCE_ERROR, 0, 0, false};
        }

        int8_t media_source = MEDIA_SOURCE_NO_MEDIA;
        std::wstring media_title = L"";
        uint16_t media_duration = 0;
        uint16_t media_currentTime = 0;
                    ...
                    if (SUCCEEDED(op->GetResults(&manager)) && manager) {
                        ...
                        if (SUCCEEDED(manager->GetCurrentSession(&session)) && session) {
                            ...
                            if (SUCCEEDED(session->GetPlaybackInfo(&playback)) && playback) {
                                ...
                                media_isPlaying = pStatus == GlobalSystemMediaTransportControlsSessionPlaybackStatus_Playing;
                            }
                            ...
                            if (SUCCEEDED(session->GetTimelineProperties(&timeline)) && timeline) {
                                ...
                                if (SUCCEEDED(timeline->get_Position(&pos)) && SUCCEEDED(timeline->get_EndTime(&end)) &&
                                    ...
                                    if (media_isPlaying) {
                                        ...
                                        media_currentTime = posSec + (uint32_t) ((nowTicks.QuadPart - lastTicks.QuadPart) / 10000000);
                                    }
                                    ...
                                    if (SUCCEEDED(propOp->GetResults(&props)) && props) {
                                        ...
                                        if (title.GetRawBuffer(nullptr)) {
                                            ...
                                            if (kickPos != std::wstring::npos) {
                                                ...
                                                media_title = fullTitle.substr(0, std::min(64, static_cast<int>(kickPos)));
                                            }
                                            else {
