...
static constexpr const char* kPluginId = "windowsupdate";

QString WindowsUpdateUi::stateToText(int32_t state) {
    switch (state) {
...
QString WindowsUpdateUi::configPath() {
...
WindowsUpdateUi::WindowsUpdateUi(WaHostApi* api, QWidget* parent)
    ...
    // Settings
    // =====================
    auto* grpSettings = new QGroupBox("WindowsUpdate Settings", this);
    ...
    spinInterval_->setRange(60 * 1000, 7 * 24 * 60 * 60 * 1000);
    spinInterval_->setSingleStep(60 * 1000);
...
void WindowsUpdateUi::loadFromDisk() {
    QFile f(configPath());
    if (!f.exists()) {
        spinInterval_->setValue(60 * 60 * 1000);
        return;
    }
    if (!f.open(QIODevice::ReadOnly)) {
        QMessageBox::warning(this, "WindowsUpdate", "Failed to open config file for reading.\n" + configPath());
        spinInterval_->setValue(60 * 60 * 1000);
        return;
    }
    ...
    if (perr.error != QJsonParseError::NoError || !doc.isObject()) {
        QMessageBox::warning(this, "WindowsUpdate", "Config JSON parse error.\n" + perr.errorString());
        spinInterval_->setValue(60 * 60 * 1000);
        return;
    }

    const QJsonObject root = doc.object();
    spinInterval_->setValue(root.value("intervalMs").toInt(60 * 60 * 1000));
}

bool WindowsUpdateUi::saveToDisk() {
    ...
    if (!sf.open(QIODevice::WriteOnly)) {
        QMessageBox::warning(this, "WindowsUpdate", "Failed to open config file for writing.\n" + configPath());
        return false;
    }
    ...
    if (!sf.commit()) {
        QMessageBox::warning(this, "WindowsUpdate", "Failed to commit config file changes.\n" + configPath());
        return false;
    }
...
void WindowsUpdateUi::refreshStatus() {
