...
static constexpr const char* kPluginId = "restartwatcher";

QString RestartWatcherUi::stateToText(int32_t state) {
    switch (state) {
...
QString RestartWatcherUi::configPath() {
...
RestartWatcherUi::RestartWatcherUi(WaHostApi* api, QWidget* parent)
    ...
    // Settings
    // =====================
    auto* grpSettings = new QGroupBox("RestartWatcher Settings", this);
    ...
    spinInterval_->setRange(1000, 24 * 60 * 60 * 1000);
...
void RestartWatcherUi::loadFromDisk() {
    QFile f(configPath());
    if (!f.exists()) {
        spinInterval_->setValue(60 * 1000);
        return;
    }
    if (!f.open(QIODevice::ReadOnly)) {
        QMessageBox::warning(this, "RestartWatcher", "Failed to open config file for reading.\n" + configPath());
        spinInterval_->setValue(60 * 1000);
        return;
    }
    ...
    if (perr.error != QJsonParseError::NoError || !doc.isObject()) {
        QMessageBox::warning(this, "RestartWatcher", "Config JSON parse error.\n" + perr.errorString());
        spinInterval_->setValue(60 * 1000);
        return;
    }

    const QJsonObject root = doc.object();
    spinInterval_->setValue(root.value("intervalMs").toInt(60 * 1000));
}

bool RestartWatcherUi::saveToDisk() {
    ...
    if (!sf.open(QIODevice::WriteOnly)) {
        QMessageBox::warning(this, "RestartWatcher", "Failed to open config file for writing.\n" + configPath());
        return false;
    }
    ...
    if (!sf.commit()) {
        QMessageBox::warning(this, "RestartWatcher", "Failed to commit config file changes.\n" + configPath());
        return false;
    }
...
void RestartWatcherUi::refreshStatus() {
