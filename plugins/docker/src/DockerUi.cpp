...
static constexpr const char* kPluginId = "docker";

QString DockerUi::stateToText(int32_t state) {
    switch (state) {
...
DockerUi::DockerUi(WaHostApi* api, QWidget* parent)
    ...
    // Settings
    auto* grpSettings = new QGroupBox("Docker Settings", this);
    ...
    spinInterval_->setRange(1000, 60 * 1000);
    spinInterval_->setSingleStep(1000);
    spinInterval_->setSuffix(" ms");

    auto* lblPath = new QLabel("docker.exe:", grpSettings);
    editDockerPath_ = new QLineEdit(grpSettings);
    editDockerPath_->setPlaceholderText("docker (PATH) or full path");
    ...
    settingsLayout->addWidget(lblPath);
    settingsLayout->addWidget(editDockerPath_, 1);
    settingsLayout->addSpacing(12);
...
void DockerUi::loadFromDisk() {
    QFile f(configPath());
    if (!f.exists()) {
        spinInterval_->setValue(3000);
        editDockerPath_->setText("docker");
        return;
    }
    if (!f.open(QIODevice::ReadOnly)) {
        QMessageBox::warning(this, "Docker", "Failed to open config file for reading.\n" + configPath());
        spinInterval_->setValue(3000);
        editDockerPath_->setText("docker");
        return;
    }
    ...
    if (perr.error != QJsonParseError::NoError || !doc.isObject()) {
        QMessageBox::warning(this, "Docker", "Config JSON parse error.\n" + perr.errorString());
        spinInterval_->setValue(3000);
        editDockerPath_->setText("docker");
        return;
    }
    ...
    editDockerPath_->setText(root.value("dockerPath").toString("docker"));
}

bool DockerUi::saveToDisk() {
    {
    ...
    root.insert("dockerPath", editDockerPath_->text().trimmed().isEmpty() ? "docker" : editDockerPath_->text().trimmed());

    QSaveFile sf(configPath());
    if (!sf.open(QIODevice::WriteOnly)) {
        QMessageBox::warning(this, "Docker", "Failed to open config file for writing.\n" + configPath());
        return false;
    }
    ...
    if (!sf.commit()) {
        QMessageBox::warning(this, "Docker", "Failed to commit config file changes.\n" + configPath());
        return false;
    }
